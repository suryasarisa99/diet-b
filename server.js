const express = require("express");
const app = express();
const session = require("express-session");
require("dotenv").config();
const axios = require("axios");
const request = require("request");
const fs = require("fs/promises");

// const axiosCookieJarSupport = require("axios-cookiejar-support").wrapper;
// const tough = require("tough-cookie");
// axiosCookieJarSupport(axios);
// const cookieJar = new tough.CookieJar();
// console.log(axiosCookieJarSupport);

const use = require("./utils/use");
const dbConnection = require("./utils/db");

const cookieExpireHours = 6;

const { getAttendence } = require("./lib/std/attendence");
const { getCookie } = require("./lib/std/utils");
const { getGraph } = require("./lib/std/graph");
use(app);
// dbConnection();

// * Routes
app.use("/auth", require("./routes/auth"));

let defaultCookie =
  "ASP.NET_SessionId=wc5m2gvo3pqqrg55pfg2dhyc;frmAuth=";

app.post("/login", (req, res) => {
  const { user, password } = req.body;
  console.log("IN: Login ", user, password);
  getCookie(user, password).then((cookie) => {
    console.log(cookie);
    if (cookie.cookie.includes("frmAuth=")) return res.json(cookie);
    else return res.status(401).json({ message: "Invalid Credentials" });
  });
});

app.post("/graph", (req, res) => {
  const {
    rollNo,
    cookie,
    expire,
    user,
    password,
    excludeOtherSubjects = false,
  } = req.body;
  console.log("graph requested");
  // console.log(req.body);

  let diff = new Date() - new Date(+expire);

  if (diff < cookieExpireHours * 60 * 60 * 1000) {
    console.log(
      "graph: cookie has valid for : ",
      cookieExpireHours - (diff / 1000 / 60 / 60).toFixed(2) + " hrs"
    );
    console.log(expire);
    getGraph({
      cookie,
      rollNo,
      excludeOtherSubjects,
      // from,
      // to
    }).then((data) => {
      console.log("graph response sent");
      return res.json({ arr: data });
    });
  } else {
    console.log(
      "graph: Cookie is Expired by : ",
      cookieExpireHours - (diff / 1000 / 60 / 60).toFixed(2) + " hrs"
    );
    getCookie(user, password).then((newCookie) => {
      getGraph({
        cookie: newCookie.cookie,
        rollNo,
        excludeOtherSubjects,
        // from,
        // to
      }).then((data) => {
        console.log("graph response sent");
        return res.json({
          arr: data,
          ...newCookie,
        });
      });
    });
  }
});
app.post("/attendance", async (req, res) => {
  const {
    cookie = defaultCookie,
    rollNo,
    from,
    to,
    user,
    password,
    excludeOtherSubjects = true,
    expire,
  } = req.body;

  let fromDate = "",
    toDate = "";
  if (from) fromDate = ToIstTime(+from);
  if (to) toDate = ToIstTime(+to);
  let diff = new Date() - new Date(+expire);

  // await new Promise((resolve, reject) => setTimeout(resolve, 5000));

  if (diff < cookieExpireHours * 60 * 60 * 1000) {
    console.log(
      "cookie has valid for : ",
      cookieExpireHours - (diff / 1000 / 60 / 60).toFixed(2) + " hrs"
    );
    getAttendence({
      cookie,
      rollNo,
      from: fromDate,
      to: toDate,
      excludeOtherSubjects,
    }).then((html) => {
      if (html.total.held == "Password") {
        console.log("att: Cookie Expired, but time is left");
        getCookie(user, password).then((newCookie) => {
          console.log(newCookie);
          getAttendence({
            cookie: newCookie.cookie,
            rollNo,
            from: fromDate,
            to: toDate,
            excludeOtherSubjects,
          }).then((html) => {
            return res.json({
              ...html,
              ...newCookie,
            });
          });
        });
      } else {
        return res.json(html);
      }
    });
  } else {
    console.log(
      "Cookie is Expired by : ",
      cookieExpireHours - (diff / 1000 / 60 / 60).toFixed(2) + " hrs"
    );
    getCookie(user, password).then((newCookie) => {
      getAttendence({
        cookie: newCookie.cookie,
        rollNo,
        from: fromDate,
        to: toDate,
        excludeOtherSubjects,
      }).then((html) => {
        return res.json({
          ...html,
          ...newCookie,
        });
      });
    });
  }
});

// app.get("/test", (req, res) => {
// getCookie("21u41a0546", "18122001")
//   .then((cookie) => {
//     console.log(cookie);
//     res.json(cookie);
//   })
//   .catch((err) => {
//     console.log("error in geting cookie", err);
//   });
// console.log("requested: test");
// });

function ToIstTime(timestamp) {
  const date = new Date(timestamp);
  const ISTOffset = 330; // 5.5 hours in minutes
  date.setMinutes(date.getMinutes() + ISTOffset);

  const day = String(date.getUTCDate()).padStart(2, "0");
  const month = String(date.getUTCMonth() + 1).padStart(2, "0");
  const year = String(date.getUTCFullYear());

  return `${day}/${month}/${year}`;
}

app.listen(process.env.PORT || 3000);

// 'Fri Mar 08 2024 02:56:22 GMT+0530 (India Standard Time)'
// {
// "cookie": "ASP.NET_SessionId=k20ejmrea3cvh4450voejz45;frmAuth=",
// "role": "teacher"
// }

// end: 4:2
